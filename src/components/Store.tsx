  ...
  return (
    <div className="min-h-screen bg-[#04020b] text-white p-4 lg:p-8 relative overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          {affiliateCode && (
            <div className="hidden md:flex items-center gap-2 px-3 py-1 bg-green-500/10 rounded-full border border-green-500/20">
              ...
              <span className="text-[10px] font-bold text-green-400 uppercase tracking-wider">Referência: {affiliateCode}</span>
            </div>
          )}
          <button 
      ...
      <header className="text-center mb-16 relative z-10">
        ...
        >
          Aproveite as ofertas exclusivas da Valtrix. Itens entregues diretamente na sua conta com segurança garantida via Mercado Pago.
        </motion.p>
      </header>
          ...
          products.map((product, idx) => (
            ...
            >
              <div className="absolute top-0 right-0 p-6">
                <div className="w-10 h-10 bg-black/40 backdrop-blur-md rounded-xl flex items-center justify-center border border-white/5 group-hover:border-purple-500/30 transition-colors">
              ...
              <div className="space-y-2 mb-8">
                <h3 className="text-xl font-bold tracking-tight group-hover:text-purple-400 transition-colors line-clamp-1">{product.name}</h3>
                <p className="text-sm text-muted-foreground line-clamp-3 min-h-[60px]">{product.description || 'Nenhuma descrição fornecida pelo Roblox.'}</p>
              </div>

              <div className="mt-auto flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Preço</p>
